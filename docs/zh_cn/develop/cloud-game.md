...
3. 排队 / 串流加载；
4. 串流通了之后画面归**游戏本体**，还有游戏登录页的「进入游戏」、开服公告、月卡领取三道关。

所以缺了这一段，任务只会干等到 `ready_timeout` 再报「未能进入游戏」，而真因是没人点按钮。

第 4 步是后来补的，也是「排队明明通过了却仍进不了游戏」的直接原因：早先把这个阶段归进「排队 / 加载中」一路干等到超时。
...
- **`SceneAnyEnterWorld` 只在确认已到游戏侧后才调用**。理由见下。

### 游戏登录页复用 SceneAnyEnterWorld，但不在排队页调用

游戏登录页的「进入游戏」、公告、月卡这三道关，仓库里早有实现：`Interface/Scene/Scene.json` 的公开接口 `SceneAnyEnterWorld` 把它们连同两种加载页一起串好了。所以 `enter.py` 直接复用，不重写一套（规范也要求只用公开接口，禁止引用 `__ScenePrivate*`）。

但它的 next 链里有兜底的 `[JumpBack]__ScenePrivateAnyExit`，动作是**按 ESC**。排队界面属于云启动器而不是游戏，在那上面按 ESC 有可能直接取消排队——把等了很久的队白排掉，远比多等一会儿严重。

所以只在拿到「确实已经在游戏侧」的正面证据之后才调用它，排队/加载阶段一律不碰，由 `max_queue_time` 兜底。`verify_cloudgame.py` 对两个方向都有断言：登录页必须调用，排队页/主页/确认弹窗必须不调用。

判定登录页的难点是：**它的按钮文案和启动器确认弹窗一模一样**，都是「进入游戏」。只能靠其它文本排除——确认弹窗一定同时有带倒计时的「退出启动」，主页一定有「开始游戏」。

### 识别结果判定的缺省值方向

`Context.run_recognition` 返回的是 `RecognitionDetail`，它**只有 `hit` 字段，没有 `status`**；`run_task` 返回的 `TaskDetail` 才看 `status.succeeded`。`_is_hit` 两种都要吃，按字段存在性分派。

关键是拿不准时的缺省值必须是 `False`。早先写的是 `getattr(result, "hit", True)`，一旦返回结构变化，「没进游戏」就被判成「已进游戏」——任务谎报成功，后续每日流程全部在启动器界面上空跑，而且日志里看不出异常。判成未命中的代价只是多等一会儿，两者不对称。
...
| 启动器未登录页未实机采集 | 停在启动器登录页时只会报总超时，不会明确说「需要登录」 | 同上 |
| 不做点击式登录 | 需客户端自己保持登录态 | 补登录页模板与 ROI |
| 点击链路未实机验证 | 开发机进程无输入桌面权限（见下），点击代码只过了离线断言 | 在可交互会话里跑一次 |
| **游戏登录页靠排除法判定** | 只要画面上有「进入游戏」且没有「退出启动」「开始游戏」就当作游戏登录页。若排队界面恰好也带「进入游戏」字样，会提前调用 `SceneAnyEnterWorld`（其兜底按 ESC 有取消排队的风险） | 采到排队界面的真实帧后确认其文案，必要时改成正面识别（ROI + 模板） |
