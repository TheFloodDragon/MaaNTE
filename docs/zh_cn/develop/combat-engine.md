...
启动时先看一眼画面：如果明显处于**大世界**，直接拒绝运行并在日志里说明原因。

本任务只负责战斗中出招，在大世界跑只会对着空气乱按技能键。与其让用户看着角色抽风然后自己猜哪里错了，不如直接拒绝并告诉他「请先进入战斗」。

判定用公共节点 `InWorld`（判据是同时存在 ESC 手机按钮与任务菜单按钮，两者都有现成模板与 ROI）。识别失败时**放行**——不因识别抖动挡掉一次合法的战斗。

`guard_open_world` 可关，留给想在非战斗场景跑自定义脚本的用户。
...
| `guard_open_world` | 启动时若在大世界则拒绝运行 | `true` |
...
当前任务不做场景导航，需要用户自己进入战斗。要做成「一键托管」还需要：
